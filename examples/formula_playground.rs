...
fn load_demo(name: &str) -> Sheet {
    match name {
        ...
        "lookup" => {
            ...
            sheet.set_formula("F2", "=VLOOKUP(F1,A2:C4,2,FALSE)").unwrap();
            
            sheet.set_formula("E3", "=\"Score:\"").unwrap();
            sheet.set_formula("F3", "=VLOOKUP(F1,A2:C4,3,FALSE)").unwrap();
        ...
        _ => {
            println!("Unknown demo '{}'. Available demos: basic, financial, stats, lookup", name);
